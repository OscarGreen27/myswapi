...
export class PeopleController {
  ...
  async findOne(@Param("id", ParseIntPipe) id: number): Promise<People> {
    ...
    if (!result) throw new NotFoundException(`The Person from the ${id} was not found.`);
  ...
  async create(@Body() persone: CreatePeopleDto) {
    ...
    return await this.peopleService.create(payload);
  }
  ...
  async delete(@Param("id", ParseIntPipe) id: number): Promise<boolean> {
    await fsp.rm(path.join(process.cwd(), "uploads", "people", `${String(id)}`), { recursive: true, force: true });
